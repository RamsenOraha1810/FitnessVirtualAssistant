    ...
    with sr.Microphone(device_index=1) as source:
        while True:
            ...
            try:
                ...
                if "reset counter" in text.lower().strip():
                    counter = 0
                    update_led_display(counter, WHITE)
                    print("Counter Reset.")
                if "squats" in text.lower().strip():
                    counter = 0
                    update_led_display(counter, BLUE)
                    exercise_mode = "squats"
                if "biceps" in text.lower().strip():
                    counter = 0
                    update_led_display(counter, RED)
                    exercise_mode = "biceps"
...
while cap.isOpened():
    ...
    interpreter.invoke()
    keypoints = np.array(common.output_tensor(interpreter, 0)[0][0], copy=True)

    # check our exercise mode, run respective function
    if exercise_mode == "biceps":
        bicep_counter(frame, keypoints)
    else:
        squat_counter(frame, keypoints)
